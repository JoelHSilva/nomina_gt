...
function VacacionesForm({ initialData = null, onSubmit }) { // Aseguramos que initialData es null por defecto si no se pasa
...
    observaciones: '',
    // aprobado_por, activo, fecha_creacion no editables en el form
  });
...
  useEffect(() => {
    // --- MODIFICACIÓN: Usar encadenamiento opcional para comprobar si initialData existe Y tiene id_vacaciones ---
    if (initialData?.id_vacaciones) { // Si initialData no es null/undefined Y tiene id_vacaciones (modo edición)
    // --- FIN MODIFICACIÓN ---
      setFormData({
...
      });
    } else {
         // Si no hay initialData (modo creación), resetear el formulario
         setFormData({
            id_empleado: '',
...
        // Asegurar que dias_tomados es número (si no está vacío)
        dias_tomados: formData.dias_tomados !== '' ? parseFloat(formData.dias_tomados) : null, // Usar null si está vacío
...
     // Validaciones adicionales (ej: fecha_inicio <= fecha_fin, dias_tomados > 0, id_empleado no nulo)
      if (!dataToSend.id_empleado) {
          alert('Debe seleccionar un empleado.');
          return;
      }
     if (dataToSend.dias_tomados === null || dataToSend.dias_tomados <= 0) {
         alert('La cantidad de días tomados debe ser un número mayor a 0.');
         return;
     }
...
                 disabled={!!initialData?.id_vacaciones} // Usar encadenamiento opcional
             >
...
         <Input label="Fecha Solicitud:" id="fecha_solicitud" type="date" value={formData.fecha_solicitud} onChange={handleInputChange} required disabled={!!initialData?.id_vacaciones} /> {/* Usar encadenamiento opcional */}
...
                 disabled={initialData?.estado === 'Completada' || initialData?.estado === 'Rechazada' || initialData?.estado === 'Cancelada'} //{/* Usar encadenamiento opcional */}
             >
...
            {initialData?.id_vacaciones ? 'Actualizar' : 'Crear'} Solicitud {/* Usar encadenamiento opcional */}
        </Button>
    </form>
  );
}
